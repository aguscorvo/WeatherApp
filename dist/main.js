...
import { deleteValue, getScreenWidth, spaceAvailable, sweetAlertSettings, sweetAlertWarning, } from './utils.js';
...
export let showBtn = document.querySelector('.show');
...
settingsBtn.addEventListener('click', () => sweetAlertSettings(comparisonEnabled, showAuto));
...
let temp = L.tileLayer(`https://tile.openweathermap.org/map/temp_new/{z}/{x}/{y}.png?appid=${API_KEY}`, {
...
let precipitation = L.tileLayer(`https://tile.openweathermap.org/map/precipitation_new/{z}/{x}/{y}.png?appid=${API_KEY}`, {
    attribution: 'Weather from <a href="https://openweathermap.org/">OpenWeatherMap</a>',
});
let wind = L.tileLayer(`https://tile.openweathermap.org/map/wind_new/{z}/{x}/{y}.png?appid=${API_KEY}`, {
    attribution: 'Weather from <a href="https://openweathermap.org/">OpenWeatherMap</a>',
});
...
export const setBtnShowActive = (button) => {
    if (!button.classList.contains('active')) {
        button.classList.add('active');
        showAuto = true;
    }
};
export const setBtnShowInactive = (button) => {
    if (button.classList.contains('active')) {
        button.classList.remove('active');
        showAuto = false;
    }
