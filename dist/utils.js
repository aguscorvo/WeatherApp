...
import { compareBtn, showBtn, locationInput, setBtnActive, setBtnInactive, setBtnShowActive, setBtnShowInactive } from './main.js';
...
export const spaceAvailable = (weatherNodeCounter, screenWidth) => {
    //mobile
    if (screenWidth < 720)
        if (weatherNodeCounter < 2)
            return true;
        else
            return false;
    // tablet
    if (screenWidth >= 720 && screenWidth < 1110) {
        if (weatherNodeCounter < 1)
            return true;
        else
            return false;
    }
    ...
    if (screenWidth >= 1365 && screenWidth < 1980) {
        if (weatherNodeCounter < 3)
            return true;
        else
            return false;
    }
    if (screenWidth >= 1980) {
        if (weatherNodeCounter < 4)
            return true;
        else
            return false;
    }
...
export const sweetAlertError = (message) => Swal.fire('Oops...', message, 'error');
export const sweetAlertWarning = (message) => Swal.fire('Warning', message, 'warning');
export const sweetAlertSettings = (comparisonEnabled, showAuto) => {
    ...
    Swal.fire({
        ...
        preConfirm: (comparisonEnabled) => {
            ...
            console.log('comparison = ' + comparisonBoolean + ' show = ' + showBoolean);
